...
function DemoButton({ onAuth }: { onAuth: (u: AuthUser) => void }) {
  ...
  return (
    <View style={{ paddingTop: 16, borderTopWidth: 1, borderTopColor: "#1a1a1a", width: "100%", alignItems: "center" }}>
...
function MasterLoginScreen({ onAuth, onBack }: { onAuth: (u: AuthUser) => void; onBack: () => void }) {
  const insets = useSafeAreaInsets();
...
function MasterSignupScreen({ onAuth, onBack }: { onAuth: (u: AuthUser) => void; onBack: () => void }) {
  const insets = useSafeAreaInsets();
...
function StaffLoginScreen({ onAuth, onBack }: { onAuth: (u: AuthUser) => void; onBack: () => void }) {
  const insets = useSafeAreaInsets();
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={[styles.inner, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 20 }]} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.buttons}>
          <TextInput
            style={[styles.input, { textTransform: "uppercase", letterSpacing: 4, fontFamily: "Inter_700Bold" }]}
            placeholder="BUSINESS CODE"
            placeholderTextColor="#555"
            value={businessCode}
            onChangeText={(t) => setBusinessCode(t.toUpperCase())}
            autoCapitalize="characters"
            autoCorrect={false}
          />
          <TextInput
            style={[styles.input, { textAlign: "center", fontSize: 24, letterSpacing: 10, fontFamily: "Inter_700Bold" }]}
            placeholder="••••"
            placeholderTextColor="#555"
            value={pin}
            onChangeText={setPin}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <AuthContext.Provider value={{ user, role, isMaster: role === "master", businessName, loading: false, logout, refetchUser: fetchMe }}>
      {children}
