import { useGetRetailer, useGetRetailerLedger, useAddLedgerEntry, } from "@workspace/api-client-react";
...
function LedgerRow({ entry }) {
    ...
    return (<View style={[styles.ledgerRow, { borderBottomColor: colors.border }]}>
      <View style={[
            styles.ledgerIcon,
            { backgroundColor: isPayment ? "#22c55e22" : "#f59e0b22" },
        ]}>
        <Feather name={isPayment ? "arrow-down-left" : "arrow-up-right"} size={16} color={isPayment ? "#22c55e" : "#f59e0b"}/>
      </View>
      <View style={styles.ledgerMeta}>
        <Text style={[styles.ledgerType, { color: colors.foreground }]}>
          {isPayment ? "Payment Received" : "Sale"}
        </Text>
        {entry.note ? (<Text style={[styles.ledgerNote, { color: colors.mutedForeground }]} numberOfLines={1}>
            {entry.note}
          </Text>) : null}
...
export default function RetailerDetailScreen() {
    ...
    const retailerId = parseInt(id !== null && id !== void 0 ? id : "0", 10);
    ...
    useEffect(() => {
        if (retailer)
            navigation.setOptions({ title: retailer.name });
    }, [retailer, navigation]);
    ...
    const utilizationPct = retailer && retailer.creditLimit > 0
        ? Math.min(1, retailer.outstanding / retailer.creditLimit)
        : 0;
    return (<View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 100 }} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={colors.primary}/>} showsVerticalScrollIndicator={false}>
        {isLoading && !retailer ? (<ActivityIndicator color={colors.primary} style={{ marginTop: 60 }}/>) : retailer ? (<>
            <View style={[styles.profileCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              {retailer.isOverdue && (<View style={[styles.overdueTag, { backgroundColor: "#ef444422" }]}>
            ...
            {!ledger || ledger.length === 0 ? (<View style={styles.emptyState}>
              ...
              </View>) : ([...ledger].reverse().map((entry) => (<LedgerRow key={entry.id} entry={entry}/>)))}
      ...
      {retailer && (<View style={[
          ...
          <Pressable style={styles.fabInner} onPress={() => setModalVisible(true)}>
      ...
      <Modal visible={modalVisible} animationType="slide" presentationStyle="formSheet" onRequestClose={() => setModalVisible(false)}>
        ...
        ]} placeholder="0" placeholderTextColor={colors.mutedForeground} value={amount} onChangeText={setAmount} keyboardType="decimal-pad" autoFocus/>
            <Text style={[styles.inputLabel, { color: colors.mutedForeground, marginTop: 16 }]}>NOTE (optional)</Text>
            <TextInput style={[
            styles.input,
            { backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular" },
        ]} placeholder="e.g. Bank transfer" placeholderTextColor={colors.mutedForeground} value={note} onChangeText={setNote}/>
            <Pressable style={[styles.confirmBtn, { backgroundColor: colors.primary, marginTop: 24 }]} onPress={handlePayment} disabled={submitting}>
              {submitting ? (<ActivityIndicator color={colors.primaryForeground} size="small"/>) : (<Text style={[styles.confirmBtnText, { color: colors.primaryForeground }]}>Confirm Payment</Text>)}
            </Pressable>
          </View>
