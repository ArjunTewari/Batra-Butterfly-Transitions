import { useListInvoices, useConfirmInvoice, useDeleteInvoice, } from "@workspace/api-client-react";
...
export default function InvoiceDetailScreen() {
    ...
    const invoice = (invoices !== null && invoices !== void 0 ? invoices : []).find((inv) => inv.id === invoiceId);
    useEffect(() => {
        if (invoice)
            navigation.setOptions({ title: invoice.invoiceNumber });
    }, [invoice, navigation]);
    async function handleConfirm() {
        if (!invoice)
            return;
        Alert.alert("Confirm Invoice", "This will update stock, ledger, and commissions. Continue?", [
    ...
    function buildChallanText(inv) {
        ...
        const itemLines = inv.items.map((item) => {
            var _a;
            const name = ((_a = item.productName) !== null && _a !== void 0 ? _a : item.articleCode).slice(0, 20).padEnd(20);
    ...
    async function handleDelete() {
        Alert.alert("Delete Invoice", "This will permanently delete this draft. Continue?", [
            { text: "Cancel", style: "cancel" },
            {
                text: "Delete",
                style: "destructive",
    ...
    return (<View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 120 }} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={colors.primary}/>} showsVerticalScrollIndicator={false}>
        <View style={[styles.headerCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          {invoice.notes && (<View style={[styles.noteBox, { backgroundColor: colors.secondary, borderColor: colors.border }]}>
        ...
        <View style={[styles.itemsCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          {invoice.items.map((item, i) => (<View key={item.id} style={[
                styles.itemRow,
                {
      ...
      <View style={[
        ...
        <Pressable style={[styles.deleteBtn, { borderColor: colors.border, backgroundColor: colors.secondary }]} onPress={handleShare}>
          <Feather name="share-2" size={20} color={colors.foreground}/>
        </Pressable>

        {isDraft ? (<>
            <Pressable style={[styles.deleteBtn, { borderColor: colors.border, backgroundColor: colors.secondary }]} onPress={handleDelete} disabled={deleting}>
              {deleting ? (<ActivityIndicator color={colors.destructive} size="small"/>) : (<Feather name="trash-2" size={20} color={colors.destructive}/>)}
            </Pressable>
            {isMaster ? (<Pressable style={[styles.confirmBtn, { backgroundColor: colors.primary }]} onPress={handleConfirm} disabled={confirming}>
                {confirming ? (<ActivityIndicator color={colors.primaryForeground} size="small"/>) : (<>
