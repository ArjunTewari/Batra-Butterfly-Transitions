...
function StockCard({ item }: { item: StockItem }) {
  ...
  return (
    ...
    >
      ...
      {margin > 0 && (
        <View style={[styles.marginRow, { borderTopColor: colors.border }]}>
...
export default function StockScreen() {
  ...
  const filtered = (stock ?? [])
    .filter((item) => {
      if (filterLow && item.currentStock > LOW_STOCK_THRESHOLD) return false;
      if (search) {
        return (
  ...
  const lowCount = (stock ?? []).filter((i) => i.currentStock <= LOW_STOCK_THRESHOLD).length;
  ...
  const totalValue = (stock ?? []).reduce((s, i) => s + i.price * i.currentStock, 0);
  const totalCost = (stock ?? []).reduce((s, i) => s + (i.purchasePrice || 0) * i.currentStock, 0);
        ...
        {totalValue > 0 && (
          <View style={[styles.valueCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={[styles.valueCount, { color: colors.mutedForeground }]}>
              {(stock ?? []).length} products · Cost {fmt(totalCost)} · Margin {fmt(totalMargin)}
            </Text>
          </View>
        )}
          ...
          >
            ...
            >
              {createProduct.isPending ? (
                <Text style={[styles.actionBtnText, { color: colors.primaryForeground }]}>Creating...</Text>
              ) : (
                <>
