...
export default function AccountTab() {
  ...
  const saveProfile = async () => {
    ...
    if (businessName !== account?.businessName) body.businessName = businessName;
    ...
    try {
      ...
      if (!res.ok) { Alert.alert("Error", data.error ?? "Save failed"); return; }
      setAccount(data);
  ...
  const addStaff = async () => {
    ...
    try {
      ...
      if (!res.ok) { Alert.alert("Error", created.error ?? "Failed"); return; }
      ...
      await fetchAll();
      setNewName(""); setNewCommission("5"); setNewPin(""); setShowAddStaff(false);
  ...
  const setPin = async (staffId: number) => {
    ...
    if (!pin || pin.length < 4) { Alert.alert("Error", "PIN must be at least 4 digits"); return; }
    try {
      ...
      if (!res.ok) { Alert.alert("Error", "Failed to set PIN"); return; }
  ...
  const removePin = (staffId: number) => {
          ...
          <View style={s.section}>
            ...
            <View style={s.card}>
              ...
              <View>
                ...
                <TextInput style={[s.input, s.inputDisabled]} value={account?.email ?? ""} editable={false} />
              </View>
              <View>
        ...
        {isMaster && (
          <View style={s.section}>
            ...
            <View style={s.card}>
              {/* Add new staff */}
              <Pressable
                style={({ pressed }) => [s.row, { backgroundColor: "#111", borderWidth: 1, borderColor: colors.border, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, opacity: pressed ? 0.7 : 1 }]}
              ...
              <View style={s.divider} />
              {staff.length === 0 ? (
                <Text style={{ color: colors.mutedForeground, fontSize: 13, fontFamily: "Inter_400Regular" }}>No staff members yet.</Text>
              ) : (
                staff.map((member, idx) => (
                  <View key={member.id} style={[s.staffRow, idx === staff.length - 1 && { borderBottomWidth: 0, paddingBottom: 0 }]}>
                    ...
                    <View style={s.row}>
                      <TextInput
                        style={[s.input, { flex: 1, textAlign: "center", letterSpacing: 6, fontFamily: "Inter_700Bold" }]}
                        placeholder="New PIN"
                        placeholderTextColor={colors.mutedForeground}
                        value={pinMap[member.id] ?? ""}
                        onChangeText={(t) => setPinMap((p) => ({ ...p, [member.id]: t.replace(/\D/g, "").slice(0, 8) }))}
                        keyboardType="number-pad"
                        secureTextEntry
                        maxLength={8}
                      />
                      <Pressable
                        style={({ pressed }) => [s.primaryBtn, { flex: 0, paddingHorizontal: 14, opacity: pressed || !pinMap[member.id] ? 0.6 : 1 }]}
                        onPress={() => setPin(member.id)}
                        disabled={!pinMap[member.id]}
                      >
                        <Text style={s.primaryBtnText}>Set</Text>
                      </Pressable>
                      {member.hasPin && (
                        <Pressable style={s.dangerBtn} onPress={() => removePin(member.id)}>
                ...
                {aiUsage.byFeature.length > 0 && (
                  <>
                    ...
                    {aiUsage.byFeature.map((f) => (
                      <View key={f.feature} style={s.featureRow}>
