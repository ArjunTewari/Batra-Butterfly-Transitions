...
export default function PaymentClearanceScreen() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Payment Cleared",
        `\u20B9${amt.toLocaleString("en-IN")} cleared from ${result.retailer?.name ?? retailerName} to ${result.vendor?.name ?? vendorName}`
      );
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Failed to generate payment";
      const apiMsg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error;
      Alert.alert("Error", apiMsg ?? msg);
    }
  };
      ...
      >
        <View style={[styles.modal, { backgroundColor: colors.background }]}>
          ...
          >
            ...
            <TextInput
              style={[styles.input, { backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular", minHeight: 80 }]}
            ...
            <Pressable
              ...
              disabled={!canSubmit || createClearance.isPending}
            >
