import { useGetStaffPerformance, useCreateStaff, useListStaff, } from "@workspace/api-client-react";
...
function StaffCard({ member, rank }) {
    ...
    const rankColor = (_a = rankColors[rank]) !== null && _a !== void 0 ? _a : colors.mutedForeground;
    const isTop = rank < 3;
    return (<View style={[
            styles.card,
            {
...
export default function StaffScreen() {
    ...
    const sorted = [...(performance !== null && performance !== void 0 ? performance : [])].sort((a, b) => b.totalSales - a.totalSales);
    ...
    return (<View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingTop: topPad + 12, paddingBottom: bottomPad + 16 }} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={colors.primary}/>} showsVerticalScrollIndicator={false}>
        <View style={styles.headerRow}>
          <Text style={[styles.title, { color: colors.foreground }]}>Staff</Text>
          {(staffList !== null && staffList !== void 0 ? staffList : []).length > 0 && (<View style={[styles.badge, { backgroundColor: colors.secondary }]}>
              <Text style={[styles.badgeText, { color: colors.mutedForeground }]}>
                {(staffList !== null && staffList !== void 0 ? staffList : []).length} members
              </Text>
            </View>)}
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.monthRow}>
          {MONTHS.map((m, i) => {
            const idx = i + 1;
            return (<Pressable key={idx} style={[
                    styles.monthBtn,
                    {
        ...
        {totalSales > 0 && (<View style={styles.summaryRow}>
        ...
        {sorted.length === 0 && !isLoading ? (<View style={styles.emptyState}>
          ...
          </View>) : (sorted.map((member, rank) => (<StaffCard key={member.id} member={member} rank={rank}/>)))}
      </ScrollView>
      ...
      <Modal visible={modalVisible} animationType="slide" presentationStyle="formSheet" onRequestClose={() => setModalVisible(false)}>
        <View style={[styles.modal, { backgroundColor: colors.background }]}>
          ...
          <ScrollView contentContainerStyle={styles.modalBody} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <TextInput style={[styles.input, { backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular" }]} placeholder="Enter staff name" placeholderTextColor={colors.mutedForeground} value={staffName} onChangeText={setStaffName} autoCapitalize="words"/>

            <Text style={[styles.sectionLabel, { color: colors.mutedForeground, marginTop: 16 }]}>
              COMMISSION RATE (%)
            </Text>
            <TextInput style={[styles.input, { backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular" }]} placeholder="e.g. 5.0" placeholderTextColor={colors.mutedForeground} value={commissionRate} onChangeText={setCommissionRate} keyboardType="decimal-pad"/>
        ...
        ]} onPress={handleCreate} disabled={!canSubmit || createStaff.isPending}>
              {createStaff.isPending ? (<Text style={[styles.actionBtnText, { color: colors.primaryForeground }]}>Adding...</Text>) : (<>
