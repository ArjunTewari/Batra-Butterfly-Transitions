import { useListInvoices, useAnalyzeInvoiceImage, useCreateInvoice, useConfirmInvoice, useListRetailers, useListStaff, } from "@workspace/api-client-react";
...
function InvoiceCard({ invoice }) {
    ...
    const sc = (_a = statusColors[invoice.status]) !== null && _a !== void 0 ? _a : statusColors.draft;
...
export default function InvoicesScreen() {
    ...
    const [selectedRetailerId, setSelectedRetailerId] = useState(0);
    ...
    const filtered = (invoices !== null && invoices !== void 0 ? invoices : []).filter((inv) => filter === "all" ? true : inv.status === filter);
    const draftCount = (invoices !== null && invoices !== void 0 ? invoices : []).filter((i) => i.status === "draft").length;
    async function handlePickImage() {
        ...
        if (!result.canceled && result.assets[0].base64) {
            await handleAnalyze(result.assets[0].base64, (_a = result.assets[0].mimeType) !== null && _a !== void 0 ? _a : "image/jpeg");
        }
    }
    ...
    async function handleAnalyze(base64, mimeType) {
        setStep("analyzing");
        try {
            ...
            const invoice = await createInvoice.mutateAsync({
                data: {
                    ...
                    items: analysis.items.map((item) => {
                        var _a, _b;
                        return ({
                            articleCode: (_a = item.articleCode) !== null && _a !== void 0 ? _a : "UNKNOWN",
    ...
    const manualTotal = manualItems.reduce((s, i) => s + (parseInt(i.quantity, 10) || 0) * (parseFloat(i.unitPrice) || 0), 0);
    ...
    return (<View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={{ paddingTop: topPad + 12, paddingBottom: bottomPad + 16 }} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={colors.primary}/>} showsVerticalScrollIndicator={false}>
        <View style={styles.headerRow}>
          ...
          {draftCount > 0 && (<View style={[styles.badge, { backgroundColor: "#f59e0b22" }]}>
              <Text style={[styles.badgeText, { color: "#f59e0b" }]}>{draftCount} draft</Text>
            </View>)}
        </View>

        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
          {["all", "draft", "confirmed"].map((f) => (<Pressable key={f} style={[
                styles.filterBtn,
                {
        ...
        {filtered.length === 0 && !isLoading ? (<View style={styles.emptyState}>
          ...
          </View>) : (filtered.map((inv) => <InvoiceCard key={inv.id} invoice={inv}/>))}
      </ScrollView>

      <Pressable style={[styles.fab, { backgroundColor: colors.primary }]} onPress={() => { setModalVisible(true); setStep("idle"); }}>
        <Feather name="plus" size={24} color={colors.primaryForeground}/>
      </Pressable>

      <Modal visible={modalVisible} animationType="slide" presentationStyle="formSheet" onRequestClose={() => { setModalVisible(false); resetModal(); }}>
        <View style={[styles.modal, { backgroundColor: colors.background }]}>
          ...
          <ScrollView contentContainerStyle={styles.modalBody} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            {step === "idle" && (<>
                ...
                <Pressable style={[styles.actionBtn, { backgroundColor: colors.primary }]} onPress={handleCamera}>
                ...
                <Pressable style={[styles.actionBtn, { backgroundColor: colors.secondary, borderColor: colors.border, borderWidth: 1, marginTop: 10 }]} onPress={handlePickImage}>
                ...
                <Pressable style={[styles.actionBtn, { backgroundColor: colors.secondary, borderColor: colors.border, borderWidth: 1 }]} onPress={() => setStep("manual")}>
            ...
            {step === "analyzing" && (<View style={styles.analyzing}>
            ...
            {step === "review" && analysis && (<>
                ...
                {analysis.items.map((item, i) => (<View key={i} style={[styles.itemRow, { borderBottomColor: colors.border }]}>
                    <View style={{ flex: 1 }}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 8 }}>
                  {(retailers !== null && retailers !== void 0 ? retailers : []).map((r) => (<Pressable key={r.id} style={[
                    styles.selectChip,
                    {
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 8 }}>
                  {(staff !== null && staff !== void 0 ? staff : []).map((s) => (<Pressable key={s.id} style={[
                    styles.selectChip,
                    {
                ...
                <TextInput style={[styles.notesInput, { backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular" }]} placeholder="Add a note..." placeholderTextColor={colors.mutedForeground} value={notes} onChangeText={setNotes} multiline/>

                <Pressable style={[styles.actionBtn, { backgroundColor: colors.primary, marginTop: 20 }]} onPress={handleConfirm} disabled={createInvoice.isPending}>
                  {createInvoice.isPending ? (<ActivityIndicator color={colors.primaryForeground} size="small"/>) : (<>
                ...
                <Pressable style={[styles.actionBtn, { backgroundColor: colors.secondary, borderColor: colors.border, borderWidth: 1, marginTop: 10 }]} onPress={() => setStep("idle")}>
            ...
            {step === "manual" && (<>
                ...
                <TextInput style={[styles.input, { backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular" }]} value={manualInvoiceNumber} onChangeText={setManualInvoiceNumber} placeholder="INV-..." placeholderTextColor={colors.mutedForeground}/>
                ...
                {manualItems.map((item, idx) => (<View key={idx} style={[styles.manualItemCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                    <View style={styles.manualItemHeader}>
                      <Text style={[styles.manualItemLabel, { color: colors.mutedForeground }]}>Item {idx + 1}</Text>
                      {manualItems.length > 1 && (<Pressable onPress={() => removeManualItem(idx)}>
                          <Feather name="trash-2" size={16} color="#ef4444"/>
                        </Pressable>)}
                    </View>
                    <TextInput style={[styles.input, { backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular", marginBottom: 8 }]} placeholder="Article code" placeholderTextColor={colors.mutedForeground} value={item.articleCode} onChangeText={(v) => updateManualItem(idx, "articleCode", v)} autoCapitalize="characters"/>
                    <TextInput style={[styles.input, { backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular", marginBottom: 8 }]} placeholder="Product name" placeholderTextColor={colors.mutedForeground} value={item.productName} onChangeText={(v) => updateManualItem(idx, "productName", v)}/>
                    <View style={styles.qtyRow}>
                      <TextInput style={[styles.input, { flex: 1, backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular", marginRight: 8 }]} placeholder="Qty" placeholderTextColor={colors.mutedForeground} value={item.quantity} onChangeText={(v) => updateManualItem(idx, "quantity", v)} keyboardType="number-pad"/>
                      <TextInput style={[styles.input, { flex: 2, backgroundColor: colors.secondary, borderColor: colors.border, color: colors.foreground, fontFamily: "Inter_400Regular" }]} placeholder="Unit price" placeholderTextColor={colors.mutedForeground} value={item.unitPrice} onChangeText={(v) => updateManualItem(idx, "unitPrice", v)} keyboardType="decimal-pad"/>
                    </View>
                  </View>))}

                <Pressable style={[styles.actionBtn, { backgroundColor: colors.secondary, borderColor: colors.border, borderWidth: 1, marginTop: 4 }]} onPress={addManualItem}>
                ...
                <Pressable style={[styles.actionBtn, { backgroundColor: colors.primary, marginTop: 20 }]} onPress={handleConfirmManual} disabled={createInvoice.isPending}>
                  {createInvoice.isPending ? (<ActivityIndicator color={colors.primaryForeground} size="small"/>) : (<>
                      <Feather name="check-circle" size={20} color={colors.primaryForeground}/>
                      <Text style={[styles.actionBtnText, { color: colors.primaryForeground }]}>Confirm Invoice</Text>
                    </>)}
            ...
            {step === "creating" && (<View style={styles.analyzing}>
                <ActivityIndicator color={colors.primary} size="large"/>
                <Text style={[styles.analyzingText, { color: colors.mutedForeground }]}>
                  Creating invoice...
                </Text>
