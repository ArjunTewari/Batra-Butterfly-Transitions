...
function KpiCard({ label, value, icon, accent, onPress }: KpiCardProps) {
  ...
  if (onPress) {
    return (
      <Pressable style={{ width: "47%", marginHorizontal: "1.5%" }} onPress={onPress}>
        {content}
      </Pressable>
    );
  }
        ...
        <>
          ...
          <View style={styles.kpiGrid}>
            ...
            <KpiCard
              ...
              icon="credit-card"
              accent="#f59e0b"
              onPress={() => router.push("/(tabs)/retailers")}
            />
            <KpiCard
              label="Retailers"
              value={String(summary?.totalRetailers ?? 0)}
              icon="users"
              onPress={() => router.push("/(tabs)/retailers")}
            />
            <KpiCard
              label="Overdue"
              value={String(summary?.overdueRetailerCount ?? 0)}
              icon="alert-circle"
              accent={summary?.overdueRetailerCount ? "#ef4444" : undefined}
              onPress={() => router.push("/(tabs)/retailers")}
            />
            <KpiCard
              label="Products"
            ...
            <KpiCard
              ...
              icon="alert-triangle"
              accent={summary?.lowStockCount ? "#f59e0b" : undefined}
              onPress={() => router.push("/(tabs)/stock")}
            />
          </View>
          ...
          {!activity || activity.length === 0 ? (
            ...
            activity.slice(0, 15).map((item) => {
              const meta = activityIcon[item.type] ?? { icon: "circle", color: colors.mutedForeground };
              return (
                <View
                  key={item.id}
                  style={[
