...
import type { SupplierBillWithItems } from "@workspace/api-client-react";
    ...
    <View style={[styles.billCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      {expanded && (
        <View style={{ marginTop: 10 }}>
          ...
          {bill.items.map((item) => (
            <View key={item.id} style={[styles.itemRow, { backgroundColor: colors.muted }]}>
...
export default function SupplierDetailScreen() {
  ...
  const [scanStep, setScanStep] = useState<"upload" | "analyzing" | "review">("upload");
  const [scanResult, setScanResult] = useState<Record<string, unknown> | null>(null);
  const [editedItems, setEditedItems] = useState<Array<{
    articleCode: string; productName: string; quantity: string; unitPrice: string;
  }>>([]);
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      const r = analysis as { items?: Array<{ articleCode: string | null; productName: string; quantity: number; unitPrice: number | null }> };
      setScanResult(analysis as unknown as Record<string, unknown>);
      setEditedItems((r.items ?? []).map((it) => ({
  ...
  const handleSaveDraft = async () => {
    ...
    const r = scanResult as { billNumber?: string; date?: string };
    try {
      await createBill.mutateAsync({
        id: supplierId,
        data: {
          billNumber: (r.billNumber as string) ?? `BILL-${Date.now()}`,
          billDate: (r.date as string) ?? new Date().toISOString().split("T")[0],
  ...
  const handleConfirmBill = async (billId: number) => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: getGetSupplierQueryKey(supplierId) });
      const updates = (result as { stockUpdates: Array<{ articleCode: string; quantityAdded: number }> }).stockUpdates;
  ...
  return (
    <>
      <ScrollView
        ...
        contentContainerStyle={{ paddingTop: topPad + 16, paddingBottom: insets.bottom + 100, paddingHorizontal: 16 }}
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {supplier.phone ? (
              <Text style={[styles.sub, { color: colors.mutedForeground }]}>{supplier.phone}</Text>
            ) : null}
            {supplier.address ? (
              <Text style={[styles.sub, { color: colors.mutedForeground }]} numberOfLines={1}>{supplier.address}</Text>
            ) : null}
          </View>
        ...
        {supplier.stockImages.length > 0 && (
          <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            {supplier.stockImages.map((sm) => (
              <View key={sm.stockMovementId} style={[styles.smRow, { backgroundColor: colors.muted }]}>
                {sm.imageUrl ? (
                  <Image source={{ uri: sm.imageUrl }} style={styles.smImage} />
                ) : (
                  <View style={[styles.smImage, { backgroundColor: colors.border, alignItems: "center", justifyContent: "center" }]}>
          ...
          {scanStep === "review" && scanResult && (
            <ScrollView contentContainerStyle={styles.modalBody}>
              ...
              <View style={[styles.aiBox, { backgroundColor: colors.card, borderColor: colors.border }]}>
                ...
                {(scanResult.billNumber as string | null) && (
                  <Text style={[styles.aiMeta, { color: colors.mutedForeground }]}>
                    Bill #: <Text style={{ color: colors.foreground }}>{scanResult.billNumber as string}</Text>
                  </Text>
                )}
                {(scanResult.date as string | null) && (
                  <Text style={[styles.aiMeta, { color: colors.mutedForeground }]}>
                    Date: <Text style={{ color: colors.foreground }}>{scanResult.date as string}</Text>
                  </Text>
                )}
                {(scanResult.supplierName as string | null) && (
                  <Text style={[styles.aiMeta, { color: colors.mutedForeground }]}>
                    Supplier on bill: <Text style={{ color: colors.foreground }}>{scanResult.supplierName as string}</Text>
                  </Text>
                )}
                <Text style={[styles.aiMeta, { color: colors.mutedForeground }]}>
                  Confidence: <Text style={{ color: colors.foreground }}>{Math.round((scanResult.confidence as number) * 100)}%</Text>
                </Text>
              </View>

              <Text style={[styles.sectionTitle, { color: colors.foreground, marginBottom: 8 }]}>Items (edit if needed)</Text>
              {editedItems.map((item, i) => (
                <View key={i} style={[styles.editRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                  <TextInput
                    style={[styles.editInput, { color: colors.foreground, borderColor: colors.border }]}
                    value={item.articleCode}
                    onChangeText={(v) => setEditedItems((p) => p.map((it, j) => j === i ? { ...it, articleCode: v } : it))}
                    placeholder="Article"
                    placeholderTextColor={colors.mutedForeground}
                  />
                  <TextInput
                    style={[styles.editInput, { color: colors.foreground, borderColor: colors.border, flex: 2 }]}
                    value={item.productName}
                    onChangeText={(v) => setEditedItems((p) => p.map((it, j) => j === i ? { ...it, productName: v } : it))}
                    placeholder="Product"
                    placeholderTextColor={colors.mutedForeground}
                  />
                  <TextInput
                    style={[styles.editInput, { color: colors.foreground, borderColor: colors.border }]}
                    value={item.quantity}
                    onChangeText={(v) => setEditedItems((p) => p.map((it, j) => j === i ? { ...it, quantity: v } : it))}
                    placeholder="Qty"
                    placeholderTextColor={colors.mutedForeground}
                  ...
                  <TextInput
                    ...
                    onChangeText={(v) => setEditedItems((p) => p.map((it, j) => j === i ? { ...it, unitPrice: v } : it))}
                    placeholder="₹"
                    placeholderTextColor={colors.mutedForeground}
                    keyboardType="numeric"
                  />
              ...
              <Pressable
                style={[styles.confirmBtn, { backgroundColor: colors.foreground, marginTop: 20 }]}
