...
import { CreateRetailerBody, UpdateRetailerBody, GetRetailerParams, UpdateRetailerParams, DeleteRetailerParams, GetRetailerLedgerParams, AddLedgerEntryParams, AddLedgerEntryBody, } from "@workspace/api-zod";
import { requireAuth } from "../middleware/requireAuth";
const router = Router();
function computeRetailerSummary(retailer, entries) {
...
router.get("/retailers", requireAuth, async (req, res) => {
    ...
    const results = await Promise.all(retailers.map(async (r) => {
...
router.post("/retailers", requireAuth, async (req, res) => {
...
router.get("/retailers/:id", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
...
router.patch("/retailers/:id", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
    const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    ...
    if (parsed.data.creditLimit !== undefined)
        updateData.creditLimit = String(parsed.data.creditLimit);
...
router.delete("/retailers/:id", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
    const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
...
router.get("/retailers/:id/ledger", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
    const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    ...
    res.json(entries.map((e) => {
        ...
        return ({
            ...
            note: (_a = e.note) !== null && _a !== void 0 ? _a : null,
            date: e.date.toISOString(),
            createdAt: e.createdAt.toISOString(),
        });
    }));
});
router.post("/retailers/:id/ledger/entry", requireAuth, async (req, res) => {
    var _a, _b;
    const accountId = req.session.accountId;
    ...
    const [entry] = await db.insert(ledgerEntriesTable).values({
        ...
        note: (_a = parsed.data.note) !== null && _a !== void 0 ? _a : null,
    ...
    res.status(201).json({
        ...
        note: (_b = entry.note) !== null && _b !== void 0 ? _b : null,
