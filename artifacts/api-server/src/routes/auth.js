...
const router = Router();
function generateBusinessCode(businessName) {
...
router.post("/auth/staff-login", async (req, res) => {
    ...
    const staffList = await db.select().from(staffTable).where(and(eq(staffTable.accountId, account.id)));
...
router.get("/auth/me", async (req, res) => {
    ...
    if (req.session.role === "staff" && req.session.staffId) {
        ...
        res.json({
            ...
            staffName: (_b = staff === null || staff === void 0 ? void 0 : staff.name) !== null && _b !== void 0 ? _b : "Staff",
        });
        return;
    }
    res.json({
...
async function seedDemoAccount(accountId) {
