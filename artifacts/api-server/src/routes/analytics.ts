...
async function getRetailerAnalytics(retailerId: number, retailerName: string, retailerPhone: string, accountId: number) {
  const now = new Date();
  ...
  let lastOrderGap: number | null = null;
...
router.get("/analytics/top-retailers", requireAuth, async (req, res): Promise<void> => {
...
router.get("/analytics/under-buying", requireAuth, async (req, res): Promise<void> => {
  const accountId = req.session.accountId!;
  const retailers = await db.select().from(retailersTable).where(eq(retailersTable.accountId, accountId));
  const analytics = await Promise.all(retailers.map((r) => getRetailerAnalytics(r.id, r.name, r.phone, accountId)));
