...
import { db, staffTable, staffAttendanceTable, staffLoansTable, staffPaymentsTable, invoicesTable, invoiceItemsTable, } from "@workspace/db";
import { FaceScanAttendanceBody, EnrollStaffFaceBody, MarkAttendanceBody, CreateStaffLoanBody, CreateStaffPaymentBody, } from "@workspace/api-zod";
...
async function commissionItemCount(accountId, staffId, month, year) {
    var _a;
    ...
    if (filtered.length > 0) {
        ...
        totalItems = Number((_a = row === null || row === void 0 ? void 0 : row.c) !== null && _a !== void 0 ? _a : 0);
    }
...
router.get("/staff/attendance", requireAuth, async (req, res) => {
    ...
    res.json(staffList.map((s) => {
        ...
        return {
            ...
            status: (_a = r === null || r === void 0 ? void 0 : r.status) !== null && _a !== void 0 ? _a : null,
            checkInTime: (r === null || r === void 0 ? void 0 : r.checkInTime) ? r.checkInTime.toISOString() : null,
            method: (_b = r === null || r === void 0 ? void 0 : r.method) !== null && _b !== void 0 ? _b : null,
        };
    }));
});
router.post("/staff/attendance/face-scan", requireAuth, async (req, res) => {
    var _a, _b;
    const accountId = req.session.accountId;
    ...
    if (existing) {
        res.json({
            ...
            distance: best.dist,
            alreadyMarked: true,
            attendance: { id: existing.id, staffId: existing.staffId, staffName: best.staff.name, date: existing.date, status: existing.status, checkInTime: existing.checkInTime ? existing.checkInTime.toISOString() : null, method: existing.method, note: (_a = existing.note) !== null && _a !== void 0 ? _a : null },
        });
        return;
    }
    const now = new Date();
    ...
    res.json({
        ...
        distance: best.dist,
        alreadyMarked: false,
        attendance: { id: rec.id, staffId: rec.staffId, staffName: best.staff.name, date: rec.date, status: rec.status, checkInTime: rec.checkInTime ? rec.checkInTime.toISOString() : null, method: rec.method, note: (_b = rec.note) !== null && _b !== void 0 ? _b : null },
    });
});
router.post("/staff/attendance/mark", requireAuth, async (req, res) => {
    var _a;
    const accountId = req.session.accountId;
    ...
    const [rec] = await db.insert(staffAttendanceTable)
        ...
        .returning();
    res.json({ id: rec.id, staffId: rec.staffId, staffName: staff.name, date: rec.date, status: rec.status, checkInTime: rec.checkInTime ? rec.checkInTime.toISOString() : null, method: rec.method, note: (_a = rec.note) !== null && _a !== void 0 ? _a : null });
});
router.get("/staff/:id/attendance-summary", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
...
// --- Face enrollment ---
router.post("/staff/:id/face-enroll", requireAuth, async (req, res) => {
    var _a, _b;
    const accountId = req.session.accountId;
    const id = parseId(req.params.id);
    if (id === null) {
    ...
    const [updated] = await db.update(staffTable)
        .set({ faceDescriptor: parsed.data.descriptor, facePhotoUrl: (_b = (_a = parsed.data.photoUrl) !== null && _a !== void 0 ? _a : staff.facePhotoUrl) !== null && _b !== void 0 ? _b : null })
...
// --- Loans ---
router.get("/staff/:id/loans", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
    const id = parseId(req.params.id);
    if (id === null) {
        res.status(400).json({ error: "Invalid id" });
    ...
    res.json(loans.map((l) => { var _a; return ({ id: l.id, staffId: l.staffId, amount: parseFloat(l.amount), note: (_a = l.note) !== null && _a !== void 0 ? _a : null, status: l.status, date: l.date.toISOString() }); }));
});
router.post("/staff/loans", requireAuth, async (req, res) => {
    var _a, _b;
    const accountId = req.session.accountId;
    ...
    const [loan] = await db.insert(staffLoansTable).values({ accountId, staffId: parsed.data.staffId, amount: String(parsed.data.amount), note: (_a = parsed.data.note) !== null && _a !== void 0 ? _a : null, status: "active" }).returning();
    res.status(201).json({ id: loan.id, staffId: loan.staffId, amount: parseFloat(loan.amount), note: (_b = loan.note) !== null && _b !== void 0 ? _b : null, status: loan.status, date: loan.date.toISOString() });
});
router.post("/staff/loans/:id/clear", requireAuth, async (req, res) => {
    var _a;
    const accountId = req.session.accountId;
    const id = parseId(req.params.id);
    if (id === null) {
    ...
    res.json({ id: updated.id, staffId: updated.staffId, amount: parseFloat(updated.amount), note: (_a = updated.note) !== null && _a !== void 0 ? _a : null, status: updated.status, date: updated.date.toISOString() });
});
// --- Payments (clearance + master approval) ---
router.get("/staff/payments", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
    const rows = await db
    ...
    res.json(filtered.map((r) => { var _a; return ({ id: r.id, staffId: r.staffId, staffName: r.staffName, amount: parseFloat(r.amount), note: (_a = r.note) !== null && _a !== void 0 ? _a : null, status: r.status, date: r.date.toISOString(), approvedAt: r.approvedAt ? r.approvedAt.toISOString() : null }); }));
});
router.post("/staff/payments", requireAuth, async (req, res) => {
    var _a, _b;
    const accountId = req.session.accountId;
    ...
    const [payment] = await db.insert(staffPaymentsTable).values({ accountId, staffId: parsed.data.staffId, amount: String(parsed.data.amount), note: (_a = parsed.data.note) !== null && _a !== void 0 ? _a : null, status: "pending" }).returning();
    res.status(201).json({ id: payment.id, staffId: payment.staffId, staffName: staff.name, amount: parseFloat(payment.amount), note: (_b = payment.note) !== null && _b !== void 0 ? _b : null, status: payment.status, date: payment.date.toISOString(), approvedAt: null });
});
async function setPaymentStatus(req, res, newStatus) {
    var _a, _b;
    if (req.session.role !== "master") {
        res.status(403).json({ error: "Only the master account can approve or reject payments" });
        return;
    }
    const accountId = req.session.accountId;
    const id = parseId(req.params.id);
    ...
    res.json({ id: updated.id, staffId: updated.staffId, staffName: (_a = staff === null || staff === void 0 ? void 0 : staff.name) !== null && _a !== void 0 ? _a : "", amount: parseFloat(updated.amount), note: (_b = updated.note) !== null && _b !== void 0 ? _b : null, status: updated.status, date: updated.date.toISOString(), approvedAt: updated.approvedAt ? updated.approvedAt.toISOString() : null });
}
...
// --- Salary overview ---
router.get("/staff/:id/salary", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
    const id = parseId(req.params.id);
    if (id === null) {
        res.status(400).json({ error: "Invalid id" });
    ...
    const inMonth = (d) => d.getMonth() + 1 === month && d.getFullYear() === year;
