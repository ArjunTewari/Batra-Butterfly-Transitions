...
router.get("/payment-clearance", requireAuth, async (req, res) => {
...
router.post("/payment-clearance", requireAuth, async (req, res) => {
    const accountId = req.session.accountId;
...
router.post("/payment-clearance/:id/approve", requireAuth, requireMaster, async (req, res) => {
    var _a, _b;
    const accountId = req.session.accountId;
    ...
    if (!payment) {
        res.status(404).json({ error: "Payment clearance not found" });
        return;
    }
    ...
    await db.insert(ledgerEntriesTable).values({
        ...
        note: (_a = payment.notes) !== null && _a !== void 0 ? _a : `Payment clearance — vendor: ${(_b = payment.vendorName) !== null && _b !== void 0 ? _b : ""}`,
        date: now,
    });
...
router.post("/payment-clearance/:id/reject", requireAuth, requireMaster, async (req, res) => {
    const accountId = req.session.accountId;
    const paymentId = parseInt(String(req.params.id), 10);
    if (isNaN(paymentId)) {
        res.status(400).json({ error: "Invalid id" });
