...
export default function Retailers() {
    ...
    const filteredRetailers = retailers === null || retailers === void 0 ? void 0 : retailers.filter(r => r.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (<div className="space-y-6">
          ...
          <DialogContent className="bg-black border border-white/10 text-white sm:max-w-[425px]">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                ...
                <FormField control={form.control} name="creditLimit" render={({ field }) => (<FormItem>
      ...
      <div className="relative">
        ...
        <Input placeholder="Search retailers by name or phone..." className="pl-10 bg-black border-white/10 text-white w-full max-w-md" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} data-testid="input-search-retailers"/>
      </div>

      {isLoading ? (<div className="text-gray-400">Loading retailers...</div>) : (filteredRetailers === null || filteredRetailers === void 0 ? void 0 : filteredRetailers.length) === 0 ? (<div className="text-center py-12 text-gray-500 border border-white/5 rounded-lg border-dashed">
          No retailers found matching "{searchTerm}"
        </div>) : (<motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filteredRetailers === null || filteredRetailers === void 0 ? void 0 : filteredRetailers.map((retailer) => (<motion.div key={retailer.id} variants={item}>
                ...
                <Card className="bg-black border-white/10 hover:bg-white/[0.02] transition-colors cursor-pointer group h-full">
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {retailer.isOverdue && (<Badge variant="destructive" className="bg-red-500/20 text-red-400 hover:bg-red-500/30 border-red-500/50">
                    ...
                    <div className="mt-4 w-full bg-white/5 rounded-full h-1.5 overflow-hidden">
                      <div className={`h-full rounded-full ${retailer.outstanding > retailer.creditLimit ? 'bg-red-500' : 'bg-blue-500'}`} style={{ width: `${Math.min(100, Math.max(0, (retailer.outstanding / retailer.creditLimit) * 100))}%` }}/>
                    </div>
                  </CardContent>
                </Card>
              </Link>
