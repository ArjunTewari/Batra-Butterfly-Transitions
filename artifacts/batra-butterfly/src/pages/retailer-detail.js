...
import { useGetRetailer, getGetRetailerQueryKey, useGetRetailerLedger, getGetRetailerLedgerQueryKey, useAddLedgerEntry } from "@workspace/api-client-react";
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              {retailer.isOverdue && (<Badge variant="destructive" className="bg-red-500/20 text-red-400 border-red-500/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-black border-white/10">
          ...
          <CardContent>
            ...
            <div className="mt-4 w-full bg-white/5 rounded-full h-1.5 overflow-hidden">
              <div className={`h-full rounded-full ${utilization > 100 ? 'bg-red-500' : 'bg-blue-500'}`} style={{ width: `${utilization}%` }}/>
            </div>
        ...
        <Card className="bg-black border-white/10">
          ...
          <CardContent>
            ...
            {retailer.isOverdue && (<p className="text-xs text-red-400 mt-1">Overdue by {retailer.daysOverdue} days</p>)}
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        ...
        <Card className="bg-black border-white/10">
          <CardContent className="p-0">
            {loadingLedger ? (<div className="p-8 text-center text-gray-500">Loading ledger...</div>) : (ledger === null || ledger === void 0 ? void 0 : ledger.length) === 0 ? (<div className="p-8 text-center text-gray-500">No transactions yet</div>) : (<div className="divide-y divide-white/5">
                {ledger === null || ledger === void 0 ? void 0 : ledger.map((entry, i) => (<motion.div key={entry.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                    <div className="flex items-center space-x-4">
                      <div className={cn("w-10 h-10 rounded-full flex items-center justify-center", entry.type === 'payment' ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400")}>
                    ...
                    <div className={cn("font-bold", entry.type === 'payment' ? "text-green-400" : "text-red-400")}>
