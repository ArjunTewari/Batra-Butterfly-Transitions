...
export default function Staff() {
    ...
    createStaff.mutate({ data: values }, {
      onSuccess: (staff) => {
        if (capturedDescriptor) {
          enrollFace.mutate(
            { id: staff.id, data: { descriptor: capturedDescriptor } },
            { onSettled: () => { /* face enrolled */ } }
          );
        }
        setIsAddOpen(false);
        form.reset();
  ...
  const handleDialogChange = (open: boolean) => {
    if (!open) { stopCamera(); setCapturedDescriptor(null); form.reset(); }
    setIsAddOpen(open);
  };
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 pt-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {capturedDescriptor ? (
                        <div className="flex items-center gap-2 rounded-lg border border-green-500/30 bg-green-950/20 p-3">
                        ...
                        <Button type="button" variant="outline" size="sm" className="border-white/10 text-gray-300 hover:bg-white/5 w-full" onClick={startCamera}>
