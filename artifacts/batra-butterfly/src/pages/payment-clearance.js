...
import { useListPaymentClearances, useCreatePaymentClearance, useApprovePaymentClearance, useRejectPaymentClearance, getListPaymentClearancesQueryKey, } from "@workspace/api-client-react";
...
function StatusBadge({ status }) {
    ...
    return (<span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${(_a = map[status]) !== null && _a !== void 0 ? _a : map.pending}`}>
      {(_b = labels[status]) !== null && _b !== void 0 ? _b : status}
    </span>);
}
export default function PaymentClearance() {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const apiMsg = (_b = (_a = err === null || err === void 0 ? void 0 : err.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.error;
            toast({ title: "Error", description: apiMsg !== null && apiMsg !== void 0 ? apiMsg : "Failed to request clearance", variant: "destructive" });
        }
    };
    ...
    const pending = (_a = clearances === null || clearances === void 0 ? void 0 : clearances.filter((c) => c.status === "pending")) !== null && _a !== void 0 ? _a : [];
    const others = (_b = clearances === null || clearances === void 0 ? void 0 : clearances.filter((c) => c.status !== "pending")) !== null && _b !== void 0 ? _b : [];
    return (<div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input type="number" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} className="bg-white/5 border-white/10 text-white placeholder:text-gray-600 pl-7" min="0"/>
                </div>
              </div>
              <div className="space-y-2">
                <Label className="text-gray-400 text-xs uppercase tracking-wide flex items-center gap-1.5">
                ...
                <Input placeholder="e.g. Sharma Footwear" value={retailerName} onChange={(e) => setRetailerName(e.target.value)} className="bg-white/5 border-white/10 text-white placeholder:text-gray-600"/>
              </div>
              <div className="space-y-2">
                <Label className="text-gray-400 text-xs uppercase tracking-wide flex items-center gap-1.5">
                  <Building2 className="h-3 w-3"/> Vendor Name *
                </Label>
                <Input placeholder="e.g. Agra Footwear Co." value={vendorName} onChange={(e) => setVendorName(e.target.value)} className="bg-white/5 border-white/10 text-white placeholder:text-gray-600"/>
              </div>
            </div>
            <div className="space-y-2">
              <Label className="text-gray-400 text-xs uppercase tracking-wide">Notes (optional)</Label>
              <Input placeholder="e.g. Q2 stock clearance" value={notes} onChange={(e) => setNotes(e.target.value)} className="bg-white/5 border-white/10 text-white placeholder:text-gray-600"/>
            </div>
            <div className="pt-1">
              <Button className="w-full md:w-auto bg-white text-black hover:bg-gray-200 font-semibold px-8 py-5" onClick={handleGenerate} disabled={!canSubmit || isSubmitting}>
                {isSubmitting ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin"/> Submitting...</>) : (<><CreditCard className="h-4 w-4 mr-2"/> Request Clearance</>)}
              </Button>
            </div>
      ...
      {(isMaster || pending.length > 0) && !isLoading && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
            <Clock className="h-4 w-4 text-yellow-400"/> Pending Approval
            {pending.length > 0 && (<span className="ml-2 bg-yellow-500/20 text-yellow-300 text-xs px-2 py-0.5 rounded-full">{pending.length}</span>)}
          </h2>

          {pending.length === 0 ? (<Card className="bg-black border-white/10">
              <CardContent className="py-8 text-center text-gray-500 text-sm">No pending clearances</CardContent>
            </Card>) : (<div className="space-y-3">
              {pending.map((c, i) => {
                    var _a, _b;
                    return (<motion.div key={c.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
                    ...
                    <CardContent className="py-4">
                      <div className="flex items-center justify-between gap-4 flex-wrap">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-2 text-sm flex-wrap">
                              <span className="text-white font-medium">{(_a = c.retailerName) !== null && _a !== void 0 ? _a : "—"}</span>
                              <span className="text-gray-600">→</span>
                              <span className="text-gray-300">{(_b = c.vendorName) !== null && _b !== void 0 ? _b : "—"}</span>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {isMaster && (<div className="flex gap-2">
                              <Button size="sm" className="bg-green-600 hover:bg-green-500 text-white" onClick={() => handleApprove(c.id)} disabled={approveClearance.isPending || rejectClearance.isPending}>
                                <CheckCircle2 className="h-3.5 w-3.5 mr-1"/> Approve
                              </Button>
                              <Button size="sm" variant="ghost" className="border border-red-500/30 text-red-400 hover:bg-red-500/10" onClick={() => handleReject(c.id)} disabled={approveClearance.isPending || rejectClearance.isPending}>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        ...
        {isLoading ? (<div className="flex items-center justify-center py-16">
          ...
          </div>) : others.length === 0 ? (<Card className="bg-black border-white/10">
