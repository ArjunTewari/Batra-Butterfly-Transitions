...
export default function PaymentClearance() {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            toast({ title: "Error", description: apiMsg !== null && apiMsg !== void 0 ? apiMsg : msg, variant: "destructive" });
        }
    };
    ...
    return (<div className="space-y-8">
          ...
          <CardContent className="space-y-5">
            ...
            <div className="pt-1">
              <Button className="w-full md:w-auto bg-white text-black hover:bg-gray-200 font-semibold px-8 py-5" onClick={handleGenerate} disabled={!canSubmit || isSubmitting}>
                {isSubmitting ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin"/> Processing...</>) : (<><CreditCard className="h-4 w-4 mr-2"/> Generate Payment</>)}
              </Button>
            </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        ...
        {isLoading ? (<div className="flex items-center justify-center py-16">
          ...
          </div>) : !clearances || clearances.length === 0 ? (<Card className="bg-black border-white/10">
