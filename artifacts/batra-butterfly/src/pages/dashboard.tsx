      ...
      >
        ...
        <motion.div variants={item}>
          <Card className="bg-black border-white/10 shadow-xl overflow-hidden relative">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-400">{summary?.overdueRetailerCount || 0}</div>
            </CardContent>
          </Card>
        </motion.div>
            ...
            <CardContent className="p-0">
              {activity?.length === 0 ? (
                ...
                <div className="divide-y divide-white/5">
                  {activity?.map((item) => (
                    <div key={item.id} className="p-4 flex items-start space-x-4 hover:bg-white/5 transition-colors">
                      ...
                      {item.amount && (
                        <div className={`text-sm font-medium ${item.type === 'payment' ? 'text-green-400' : ''} ${item.type === 'sale' ? 'text-blue-400' : ''}`}>
          ...
          <Card className="bg-black border-white/10">
            <CardContent className="p-0">
              {topRetailers?.length === 0 ? (
                <div className="p-8 text-center text-gray-500">No top retailers data</div>
              ) : (
                <div className="divide-y divide-white/5">
                  {topRetailers?.slice(0, 5).map((retailer, index) => (
                    <div key={retailer.id} className="p-4 flex items-center space-x-4 hover:bg-white/5 transition-colors">
