...
import { useGetTopRetailers, getGetTopRetailersQueryKey, useGetUnderBuyingRetailers, getGetUnderBuyingRetailersQueryKey } from "@workspace/api-client-react";
...
export default function Analytics() {
    ...
    const { data: topRetailers, isLoading: loadingTop } = useGetTopRetailers({ days: Number(days) }, { query: { queryKey: getGetTopRetailersQueryKey({ days: Number(days) }) } });
    ...
    const chartData = (topRetailers === null || topRetailers === void 0 ? void 0 : topRetailers.map(r => ({
        name: r.name,
      ...
      <Tabs defaultValue="performance" className="w-full">
        ...
        <TabsContent value="performance" className="space-y-6">
              ...
              <div className="h-[400px] w-full mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData.slice(0, 10)} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                    ...
                    <Tooltip cursor={{ fill: '#ffffff05' }} contentStyle={{ backgroundColor: '#000', borderColor: '#ffffff20', color: '#fff', borderRadius: '8px' }} formatter={(value) => [formatCurrency(value), "Revenue"]}/>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {topRetailers === null || topRetailers === void 0 ? void 0 : topRetailers.slice(0, 4).map((retailer, i) => (<motion.div key={retailer.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                <Card className="bg-black border-white/10 hover:bg-white/[0.02] transition-colors h-full">
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className={`flex items-center px-2 py-1 rounded text-xs font-medium ${retailer.growthRate >= 0 ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}>
        ...
        <TabsContent value="risk" className="space-y-6">
          <Card className="bg-black border border-red-500/30 relative overflow-hidden">
            ...
            <CardContent>
              {loadingUnder ? (<div className="py-8 text-center text-gray-500">Analyzing patterns...</div>) : (underBuying === null || underBuying === void 0 ? void 0 : underBuying.length) === 0 ? (<div className="py-8 text-center text-green-400">No under-buying risks detected.</div>) : (<div className="divide-y divide-white/10 relative z-10">
                  {underBuying === null || underBuying === void 0 ? void 0 : underBuying.map((retailer, i) => (<motion.div key={retailer.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div>
