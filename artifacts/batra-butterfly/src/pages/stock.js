...
import { Search, Plus, Upload, PackageOpen, ShoppingBag, Trash2, } from "lucide-react";
...
export default function Stock() {
    ...
    const onSubmit = (values) => {
        createStockItem.mutate({ data: values }, {
            onSuccess: () => {
                setIsAddOpen(false);
                form.reset();
                queryClient.invalidateQueries({ queryKey: getListStockQueryKey() });
    ...
    const filteredStock = stock === null || stock === void 0 ? void 0 : stock.filter((item) => item.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link href="/stock/upload">
            <Button variant="outline" className="bg-white/5 border-white/10 text-white hover:bg-white/10 hover:text-white" data-testid="button-stock-upload">
          ...
          <Link href="/stock/sale">
            <Button variant="outline" className="bg-white/5 border-white/10 text-white hover:bg-white/10 hover:text-white" data-testid="button-stock-sale">
          ...
          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            <DialogTrigger asChild>
              <Button data-testid="button-add-stock" className="bg-white text-black hover:bg-gray-200">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                  <FormField control={form.control} name="articleCode" render={({ field }) => (<FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g. BT-402" className="bg-white/5 border-white/10 text-white" {...field} data-testid="input-stock-article"/>
                        </FormControl>
                        <FormMessage />
                      </FormItem>)}/>
                  <FormField control={form.control} name="name" render={({ field }) => (<FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Enter product name" className="bg-white/5 border-white/10 text-white" {...field} data-testid="input-stock-name"/>
                        </FormControl>
                        <FormMessage />
                      </FormItem>)}/>
                  <div className="grid grid-cols-2 gap-4">
                    <FormField control={form.control} name="purchasePrice" render={({ field }) => (<FormItem>
                          ...
                          <FormControl>
                            <Input type="number" placeholder="0" className="bg-white/5 border-white/10 text-white" {...field} data-testid="input-stock-purchase"/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>
                    <FormField control={form.control} name="price" render={({ field }) => (<FormItem>
                          ...
                          <FormControl>
                            <Input type="number" placeholder="0" className="bg-white/5 border-white/10 text-white" {...field} data-testid="input-stock-price"/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>
                  </div>
                  <FormField control={form.control} name="currentStock" render={({ field }) => (<FormItem>
                        <FormLabel>Initial Stock</FormLabel>
                        <FormControl>
                          <Input type="number" placeholder="0" className="bg-white/5 border-white/10 text-white" {...field} data-testid="input-stock-qty"/>
                        </FormControl>
                        <FormMessage />
                      </FormItem>)}/>
                  <div className="flex justify-end pt-4">
                    <Button type="submit" disabled={createStockItem.isPending} className="bg-white text-black hover:bg-gray-200" data-testid="button-submit-stock">
      ...
      <div className="relative">
        ...
        <Input placeholder="Search by name or article code..." className="pl-10 bg-black border-white/10 text-white w-full max-w-md" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} data-testid="input-search-stock"/>
      </div>
              ...
              <Card className="bg-black border-white/10 hover:bg-white/[0.02] transition-colors group h-full">
                <CardContent className="p-0">
                  <div className="h-32 bg-white/5 flex items-center justify-center relative overflow-hidden">
                    {item.imageUrl ? (<img src={item.imageUrl} alt={item.name} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity"/>) : (<PackageOpen className="h-12 w-12 text-white/10"/>)}
                  ...
                  <div className="p-4">
                    ...
                    : "—"}
                      ...
                      <div className="text-right">
                        ...
                        <p className={`font-bold text-lg ${item.currentStock <= 10 ? "text-red-400" : "text-green-400"}`}>
                          {item.currentStock}
                        </p>
                      </div>
                    </div>
                    {item.purchasePrice && (<div className="mt-3 pt-3 border-t border-white/10">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs font-medium text-green-400">
                            ...
                            <span className="text-gray-500 ml-1">
                              (
                              {Math.round(((item.price - item.purchasePrice) /
                        item.price) *
                        100)}
                              %)
                            </span>
