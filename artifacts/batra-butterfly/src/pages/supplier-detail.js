...
import { ArrowLeft, Camera, CheckCircle2, AlertCircle, Loader2, RefreshCw, Image as ImageIcon, Package, Phone, MapPin, Receipt, ChevronDown, ChevronUp, Link2, } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { formatCurrency } from "@/lib/utils";
import { Link } from "wouter";
import { useAuth } from "@/contexts/AuthContext";
function BillCard({ bill, onConfirm, confirming }) {
    const [expanded, setExpanded] = useState(false);
    const isDraft = bill.status === "draft";
    const { isMaster } = useAuth();
    return (<Card className="bg-black border-white/10">
      <CardContent className="pt-4 pb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge className={bill.status === "confirmed"
            ? "bg-green-500/10 text-green-400 border-green-500/20"
            : "bg-yellow-500/10 text-yellow-400 border-yellow-500/20"}>
              {bill.status}
            </Badge>
            {isDraft && (isMaster ? (<Button size="sm" onClick={onConfirm} disabled={confirming} className="gap-1">
                  {confirming ? <Loader2 className="h-3 w-3 animate-spin"/> : <CheckCircle2 className="h-3 w-3"/>}
                  Confirm
                </Button>) : (<span className="text-xs px-2.5 py-1 rounded-md bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 whitespace-nowrap">
                  📨 Sent for Approval
                </span>))}
        ...
        <AnimatePresence>
          {expanded && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="mt-4 space-y-2">
                ...
                {bill.items.map((item) => (<div key={item.id} className="grid grid-cols-4 gap-2 text-sm bg-white/5 rounded-lg px-3 py-2 items-center">
              ...
              {bill.notes && (<p className="text-xs text-gray-500 mt-2 px-2">Note: {bill.notes}</p>)}
            </motion.div>)}
        </AnimatePresence>
      </CardContent>
    </Card>);
...
export default function SupplierDetail() {
    ...
    const handleConfirmBill = async (billId) => {
        ...
        try {
            ...
            toast({
                ...
                description: `${result.stockUpdates.length} product(s) restocked`,
            });
        }
        catch {
    ...
    const handleCreateBill = async () => {
        ...
        try {
            await createBill.mutateAsync({
                id: supplierId,
                data: {
                    billNumber: (_a = scanResult.billNumber) !== null && _a !== void 0 ? _a : `BILL-${Date.now()}`,
                    billDate: (_b = scanResult.date) !== null && _b !== void 0 ? _b : new Date().toISOString().split("T")[0],
    ...
    return (<div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 mt-1">
            {supplier.phone && (<span className="text-xs text-gray-400 flex items-center gap-1">
                <Phone className="h-3 w-3"/> {supplier.phone}
              </span>)}
            {supplier.address && (<span className="text-xs text-gray-400 flex items-center gap-1">
                <MapPin className="h-3 w-3"/> {supplier.address}
              </span>)}
            {supplier.gstin && (<span className="text-xs text-gray-500 font-mono">GSTIN: {supplier.gstin}</span>)}
          </div>
        </div>
      ...
      {supplier.stockImages.length > 0 && (<Card className="bg-black border-white/10 mb-6">
          ...
          <CardContent>
            <div className="space-y-2">
              {supplier.stockImages.map((sm) => (<div key={sm.stockMovementId} className="flex items-center gap-3 bg-white/5 rounded-lg px-3 py-2">
                  {sm.imageUrl ? (<img src={sm.imageUrl} alt="" className="h-10 w-10 rounded object-cover flex-shrink-0"/>) : (<div className="h-10 w-10 rounded bg-white/5 flex items-center justify-center flex-shrink-0">
                  ...
                  {sm.billId ? (<Badge className="bg-green-500/10 text-green-400 border-green-500/20 text-xs gap-1">
                      <CheckCircle2 className="h-3 w-3"/> {sm.billNumber}
                    </Badge>) : (<Badge className="bg-yellow-500/10 text-yellow-400 border-yellow-500/20 text-xs">
                      Unmatched
                    </Badge>)}
                </div>))}
            </div>
      ...
      <div>
        ...
        {supplier.bills.length === 0 ? (<div className="text-center py-12 text-gray-500 border border-white/5 rounded-xl">
            <Receipt className="h-10 w-10 mx-auto mb-3 opacity-30"/>
            <p>No bills yet. Scan a supplier bill to get started.</p>
          </div>) : (<div className="space-y-3">
            {supplier.bills.map((bill) => (<BillCard key={bill.id} bill={bill} onConfirm={() => handleConfirmBill(bill.id)} confirming={confirmingId === bill.id}/>))}
          </div>)}
      </div>

      {/* Scan Dialog */}
      <Dialog open={scanOpen} onOpenChange={(open) => { if (!open) {
        setScanOpen(false);
        setScanStep("upload");
        setScanResult(null);
        setScanImage(null);
        ...
        <DialogContent className="bg-black border-white/10 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {scanStep === "upload" && (<div className="space-y-4 mt-2">
              {analyzeImage.isPending ? (<div className="flex flex-col items-center gap-3 py-12">
                  <Loader2 className="h-10 w-10 animate-spin text-white/60"/>
                  <p className="text-gray-400">Analyzing bill with AI…</p>
                </div>) : (<>
                  <div onClick={() => { var _a; return (_a = fileRef.current) === null || _a === void 0 ? void 0 : _a.click(); }} className="border-2 border-dashed border-white/10 rounded-xl p-10 text-center cursor-pointer hover:border-white/30 transition-colors">
          ...
          {scanStep === "review" && scanResult && (<div className="space-y-4 mt-2">
              {/* AI result summary */}
              <div className="bg-white/5 rounded-xl p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium">AI Extraction</p>
                  <Badge className={scanResult.confidence > 0.7
                ? "bg-green-500/10 text-green-400 border-green-500/20"
                : "bg-yellow-500/10 text-yellow-400 border-yellow-500/20"}>
              ...
              <div>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {editedItems.map((item, i) => (<div key={i} className="grid grid-cols-4 gap-2 bg-white/5 rounded-lg p-2">
                      <Input value={item.articleCode} onChange={(e) => setEditedItems((prev) => prev.map((it, j) => j === i ? { ...it, articleCode: e.target.value } : it))} placeholder="Article" className="bg-transparent border-white/10 text-xs text-white h-8"/>
                      <Input value={item.productName} onChange={(e) => setEditedItems((prev) => prev.map((it, j) => j === i ? { ...it, productName: e.target.value } : it))} placeholder="Product" className="bg-transparent border-white/10 text-xs text-white h-8"/>
                      <Input type="number" value={item.quantity} onChange={(e) => setEditedItems((prev) => prev.map((it, j) => j === i ? { ...it, quantity: parseInt(e.target.value) || 0 } : it))} placeholder="Qty" className="bg-transparent border-white/10 text-xs text-white h-8"/>
                      <Input type="number" value={item.unitPrice} onChange={(e) => setEditedItems((prev) => prev.map((it, j) => j === i ? { ...it, unitPrice: parseFloat(e.target.value) || 0 } : it))} placeholder="Price" className="bg-transparent border-white/10 text-xs text-white h-8"/>
                    </div>))}
                </div>
              ...
              <div>
                ...
                <Input value={billNote} onChange={(e) => setBillNote(e.target.value)} placeholder="Any notes about this bill..." className="mt-1 bg-white/5 border-white/10 text-white"/>
              </div>

              <div className="flex gap-3">
                <Button variant="ghost" onClick={() => { setScanStep("upload"); setScanResult(null); setScanImage(null); setEditedItems([]); }} className="flex-1">
                  <RefreshCw className="h-4 w-4 mr-2"/> Rescan
                </Button>
                <Button onClick={handleCreateBill} disabled={createBill.isPending || editedItems.length === 0} className="flex-1">
