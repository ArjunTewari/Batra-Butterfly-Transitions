...
export function Sidebar() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-black border-r border-white/10 text-white">
          ...
          {sidebarLinks.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href}>
                <div
                  className={cn(
                    "flex items-center rounded-md px-3 py-2.5 text-sm font-medium transition-all duration-200 cursor-pointer group",
                    isActive 
                      ? "bg-white/10 text-white" 
                      : "text-gray-400 hover:bg-white/5 hover:text-white",
                    link.masterOnly && "border border-yellow-500/20"
                  )}
                  data-testid={`link-sidebar-${link.label.toLowerCase().replace(/ /g, '-')}`}
                >
                  ...
                  <AnimatePresence mode="wait">
                    {!collapsed && (
                      <motion.span
                        ...
                        className={cn("overflow-hidden whitespace-nowrap", link.masterOnly && "text-yellow-300")}
                      >
                        {link.label}
                      </motion.span>
                    )}
        ...
        {!collapsed && (
          <div className="px-3 pb-1">
            <span className={cn(
              ...
              isMaster ? "bg-yellow-500/15 text-yellow-400" : "bg-white/10 text-gray-400"
            )}>
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="hidden md:block fixed inset-y-0 left-0 z-40 h-screen transition-all duration-300 ease-in-out"
      >
        <SidebarContent />
      </motion.aside>
